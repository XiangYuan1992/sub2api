export default {
  ...
  docs: {
    title: '接入文档',
    subtitle: '一个 API 密钥即可接入 Codex、Claude Code、OpenClaw、Hermes Agent。',
    baseUrlLabel: '接口地址',
    common: {
      install: '安装',
      config: '配置接入凭证',
    ...
    quickStart: {
      title: '快速开始',
      step1: '注册并登录后，前往控制台创建一个 API 密钥（以 sk- 开头）。',
      step2: '根据下方对应工具的说明，将接口地址与密钥填入配置。',
      step3: '保持账户余额充足即可正常调用，所有工具的接口地址统一使用下方地址。'
    },
    claude: {
      desc: 'Claude Code 是 Anthropic 推出的命令行 AI 编程助手。',
      skipLogin: '跳过登录验证',
      skipLoginText: '编辑或新建 ~/.claude.json，将 hasCompletedOnboarding 设为 true，跳过官方登录验证。',
      configText: '新建 ~/.claude/settings.json，将 YOUR_API_KEY 替换为你的 API 密钥。',
      verifyText: '保存后新开终端执行以下命令，模型正常返回即配置成功；也可在 Claude Code 中执行 /status 检查地址是否指向本站。'
    },
    codex: {
      desc: 'Codex 是 OpenAI 推出的终端 AI 编程助手。',
      configText: '编辑 ~/.codex/config.toml，将 YOUR_API_KEY 写入环境变量 OPENAI_API_KEY。',
      envHeading: '配置环境变量',
      envText: '将 OPENAI_API_KEY 设置为你的 API 密钥（macOS/Linux 示例，Windows 用 setx）。',
      verifyText: '新开终端执行以下命令，正常进入对话界面即配置成功。'
    },
    openclaw: {
      desc: 'OpenClaw 是一个开源的个人 AI 助手平台，需要 Node.js 22 或更高版本。',
      installText: '通过 npm 全局安装（或使用官方安装脚本）。',
      configText: '编辑 ~/.openclaw/openclaw.json，将 YOUR_API_KEY 替换为你的 API 密钥。示例禁用了网关鉴权，仅适合单机本地使用。',
      verifyText: '保存后执行以下命令重启网关使配置生效，再用 openclaw status 检查状态。'
    },
    hermes: {
      desc: 'Hermes Agent 是一款终端 AI 编程工具，通过 hermes config set 命令配置接入参数。',
      configText: '依次执行以下命令，将 YOUR_API_KEY 替换为你的 API 密钥（配置写入 ~/.hermes/config.yaml）。',
      verifyText: '配置完成后执行以下命令发送测试消息，返回正常回复即配置成功。'
    },
    faq: {
      title: '常见问题',
      q1: '支持哪些模型？',
      a1: '支持 Claude、GPT、Gemini 等主流模型，具体以价格页展示的可用分组为准。',
      q2: '提示 401 / 鉴权失败怎么办？',
      a2: '请检查 API 密钥是否复制完整、无空格且未过期，并确认接口地址填写正确。',
      q3: '密钥泄露了怎么办？',
      a3: '请立即在控制台删除该密钥并重新生成，旧密钥会立即失效。'
    }
  },
  // Home Page
  home: {
    ...
    landing: {
      ...
      heroTitle1: '连接全球顶级',
      heroTitle2: 'AI 大模型',
      heroDesc: '为全球开发者与团队而生 —— 专线高速直连、无封号风险、余额永不过期。支持支付宝 / 微信支付，低延迟调用 Claude、ChatGPT、Gemini 等主流模型。',
      ctaConsole: '进入控制台',
      ctaPricing: '查看价格',
      ctaDocs: '接入文档',
      ecosystem: '兼容生态',
      clientsTitle: '支持的设备与 AI 编程工具',
      pricingEyebrow: '定价方案',
      pricingTitle: '按量付费，按需使用',
      pricingSubtitle: '1 RMB = 1 USD，使用官方原生模型，享受更低折扣',
      valueEyebrow: '使用价值',
      valueTitle: '释放你的编程潜能，让顶尖 AI 为你写代码',
      faqEyebrow: 'FAQ',
      faqTitle: '常见问题解答',
      finalTitle: '准备好提升您的 AI 应用了吗？',
      finalDesc: '加入数以千计的开发者行列，利用稳定、专业的基础设施构建人工智能的未来。',
      recommended: '推荐',
      planPaygoName: '按量付费',
      planPaygoPrice: 'PAYGO',
      planPaygoDesc: '永不过期',
      planPaygoF1: '充值金额，获得等价人民币额度',
      planPaygoF2: '按实际使用付费',
      planPaygoF3: '永不过期',
      planPaygoCta: '立即充值',
      planClaudeName: 'Claude 按需付费',
      planClaudePrice: '1:1（RMB:USD）',
      planClaudeF1: '无需订阅，根据实际使用量灵活计费',
      planClaudeF2: '支持 Claude 全系列模型',
      planClaudeF3: '专为 Claude Code 优化',
      planGptName: 'ChatGPT 按需付费',
      planGptPrice: '1:1（RMB:USD）',
      planGptF1: '无需订阅，根据实际使用量灵活计费',
      planGptF2: '支持 OpenAI GPT 全系列模型',
      planGptF3: '专为 Codex 优化',
      planStart: '立即开始',
      value1Title: '全球直连',
      value1Desc: '无论身处何地，都能尽量稳定访问，减少等待与网络折损带来的中断。',
      value2Title: '高可用架构',
      value2Desc: '分布式架构设计与自动故障转移，保障关键时刻依然可用。',
      value3Title: '简单集成',
      value3Desc: '只需修改 API 地址即可使用，无需重写现有业务逻辑。',
      value4Title: '完整兼容',
      value4Desc: '一次购买即同时兼容官方 API 行为，保留主流模型常用能力。',
      faq1Q: '与官方有什么区别？',
      faq1A: '使用官方原生模型，接口完全兼容，只需替换 API 地址即可。计费以人民币结算，价格更低。',
      faq2Q: '如何计费？',
      faq2A: '按实际使用的 token 量计费，余额永不过期，可在使用记录中查看明细。',
      faq3Q: '支持哪些客户端？',
      faq3A: '支持 Codex、Claude Code、OpenClaw、Hermes Agent 等主流终端工具，详见接入文档。'
    },
    clients: {
      codex: 'OpenAI 推出的终端编程代理，擅长大规模重构、Bug 修复与测试生成。',
      codexTag: 'OPENAI',
      claude: 'Anthropic 官方 CLI，原生支持深度思考，写代码如自然对话般流畅。',
      claudeTag: 'ANTHROPIC',
      openclaw: '开源本地 AI 助手，运行在你自己的电脑上，通过聊天直接执行任务。',
      openclawTag: '开源 · 本地运行',
      hermes: '终端 AI 编程工具，一条命令完成配置，支持自定义模型提供商。',
